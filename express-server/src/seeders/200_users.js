...
export async function up(queryInterface) {
  ...
  const users = [
    {
      ...
      password: hashSync(process.env.ADMINUSER_PASSWORD, 10),
      createdAt: now,
      updatedAt: now,
    },
  ];
  ...
  if (createGuest) {
    users.push({
      ...
      password: hashSync(process.env.GUESTUSER_PASSWORD, 10),
      createdAt: now,
      updatedAt: now,
    });
  }
  ...
  if (createMain) {
    users.push({
      ...
      password: hashSync(process.env.MAINUSER_PASSWORD, 10),
      createdAt: now,
      updatedAt: now,
    });
  }
